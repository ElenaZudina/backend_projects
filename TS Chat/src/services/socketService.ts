...
// Сервис для настройки Socket.io  и обработки собыйтий клиентов
export class SocketService {
    ...
    public init(): void {
        this.io.on("connection", (socket: Socket) => {
            ...
            // При отключении просто логируемЖ дополнительной логики пока нет
    ...
    private handleSendMessage(socket: Socket): void {
        socket.on(
            ...
            (payload: { author: string; text: string }, callback?: (err?: string) => void) => {
                try {
                    // Сохраняем собщение через модель; она же валидирует текст
