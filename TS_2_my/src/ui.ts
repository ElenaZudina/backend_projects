...
// Утилита фороматирования даты для пользовательского вывода
...
// Создаем DOM-элемента карточки задачи
export function createTaskElement(task: Task): HTMLElement {
    ...
    priorityLabel.textContent = task.priority.toUpperCase();
    card.appendChild(priorityLabel);

    card.append(delBtn, titleEl, metaEl, priorityLabel);
    return card;
}


// Привязка обработчиков Drag & Drop к колонкам и карточкам
//  onMove - колбэк, вызывается при переносе карточки в новю колонку
export function bindDragAndDrop(
    columns: Record<Status, HTMLElement>,
    onMove: (taskId: string, to: Status) => void
): void {
    // Навешиеваем обработчики на каждую колонку
